...
### Changed
- **Yoru Shuriken now works with Sange**: the new **Yoru Shuriken** is recognized alongside Koga and the rest, so Sange auto-equips and throws it. Thanks to **Oban** for the idea and **xvs** for getting the item ID.
...
- **`/sk panel` Buffs column widened** so long buff lists stay readable.
...
- **Several Blue Mage spells wouldn't cast** — Battery Charge, Animating Wail, Magic Barrier, Occultation, Orcish Counterstance and Barrier Tusk were incorrectly locked behind **Unbridled Learning**, so Sidekick tried to pop that JA before each one and skipped the spell while it was on cooldown. On CatsEyeXI those six cast normally; only **Harden Shell**, **Pyric Bulwark** and **Carcharian Verve** still need Unbridled Learning — and those three are no longer grayed as *"not currently equipped,"* since Unbridled Learning unlocks them for you rather than the set-spell list. Thanks to **Benthere** and **Muziko** for reporting the bugs.
