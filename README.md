...
### Added
- **Sleep Targets**: the buttons in **Sleep Removal** now carry a **Sleep Targets** label, like **Group Targets** and **AOE Targets** — same idea, click a button to add or drop that person from the pool Sidekick watches for Sleep. These picks are saved with your settings rather than reset each session. There is no **ME** button, since you can't cure your own Sleep, and for the same reason the whole section is hidden when you're solo. Trusts that can't be damaged or slept at all — Moogle, Kupofried, Sakura, Star Sibyl, Brygid, Cornelia — are grayed out and can't be selected.
- **Scholar finally has an AOE heal**: Scholar gets no Curaga, so Sidekick uses the next best thing — **Accession**, the stratagem that makes your next cure hit everyone nearby. Turn on **AOE Healing** on Scholar and it works like every other job's: set the **AOE (HP%)** threshold, pick who counts with the **AOE Targets** buttons, and when the party drops below it Sidekick pops Accession and follows straight up with the biggest cure that fits, aimed at whoever is hurt worst since that is where the spell spreads from. It only spends the stratagem when a cure can actually go out right behind it — no charge burnt on a cure you couldn't afford, couldn't reach, or were silenced out of — and it budgets for the doubled MP Accession charges, so it won't commit and then come up short. **Hold AOE for Group** applies here too. Off by default, since each party heal costs you a stratagem. Thanks to **Toranko** for the feature idea.
- **Hold a Stratagem for AOE**: right-click the **Accession** row in AOE Healing for a toggle that keeps one stratagem in reserve for it. With it on, everything else that spends stratagems treats your last charge as none, so an Addendum or a discounted cure can't drain the pool the party heal is waiting on. It only applies while AOE Healing is switched on, so turning the section off releases the reserve instead of quietly holding a charge you can no longer spend. Off by default.
...
- **New floating widget (`/sk widget`)**: pops the top of the config window — your profile button, job/subjob line, the Start/Stop button, the status text and the **Track Target** button — out into a small window of its own that you can park anywhere. Your list of tracked targets stays in the config window. No title bar, no close button: `/sk widget` toggles it, drag the body to move it. While it's up those rows disappear from the config window, so you're never looking at two Start buttons. Remembered per character, so it comes back where you left it after a reload. The status text is a little shorter everywhere now (*"Automation asleep"* rather than *"Automation asleep - move to wake."*) so the widget stays small — hover it for the full explanation as before. Thanks to **Muziko** for the feature idea.
...
- **Red Mage enspells and Scholar storms can pick their own element**: right-click the enspell or storm dropdown and tick **Auto Select**. Sidekick then keeps the spell matched to the element that is actually giving you the bonus, following the tier you can cast.
  ...
  - **Storms** (*Auto Select for Weather*) follow the zone weather only, so casting your storm on top of it doubles the weather. There is no day fallback and clear skies leave your pick alone — a storm on the wrong day costs you nothing, but the wrong storm in weather does.  Thanks to **Cedwick** for the feature idea.
...
- **Monk gets Perfect Counter and Impetus**: both were missing from Monk's buff list. They join **Boost**, **Dodge**, **Focus**, **Counterstance** and **Footwork** as combat-only self buffs you can tick on — Perfect Counter at level 79, Impetus at level 88. Thanks to **Toranko** for noticing they were missing.
- **Custom song duration for level-75 Bards**: song+ duration gear makes your songs last longer than Sidekick can see, so it always waited for a song to *drop* before re-singing it. Set **Song Duration (s)** on `/sk panel` to just under your real duration and Sidekick re-sings each song on that timer instead — before it wears, so your party never loses the buff. **Troubadour** doubles the timer automatically while it's up. Set it to `0` (the default) for the old behavior. The timer starts when the song actually finishes casting, so an interrupted song is re-sung straight away, and anyone who dies has their timers dropped so a raise doesn't leave them songless. Keep the number under your real duration — while a timer is running Sidekick trusts it over what it can see, so a song stripped by a Dispel waits for that timer. Thanks to **Sleazy** for the brainstorming session behind this feature.
- **Bard gets Nightingale and Troubadour**: both merit abilities were missing from Bard's buff list. Tick them on (they start off) and Sidekick keeps them up like any other self buff — level 75, main job only, merits required.
- **Tell Sidekick about your Cure and Waltz Potency gear**: Sidekick picks the cure sized to a member's missing HP, but potency gear makes your cures land bigger than the base numbers it works from — so it could reach for a bigger tier than you needed. Set **Cure Potency +%** (or **Waltz Potency +%** on Dancer) on `/sk panel` to your gear's bonus and every heal is sized with it counted in — less overheal, less wasted MP/TP. `0` (the default) keeps the old behavior. Thanks to **Miri** for the reminder to get this going.
...
- **Tracked targets stick around when you zone**: your tracked list used to be wiped every time you crossed a zone line, so everyone you had added by hand was gone and a second box had to rebuild its party mirror from scratch. It now survives zoning — someone you've zoned away from just shows as inactive until they're near you again. Reloading the addon still clears the list.
...
### Fixed
- **Sidekick now knows a spell costs more under Accession even when it didn't press Accession**: pop Accession yourself and the next cure really costs double, but Sidekick was still budgeting the normal price — so it kept queuing a cure it couldn't pay for, over and over, until the buff wore off or your MP caught up. Same for Penury and the other cost stratagems, in reverse. It now reads the buff you actually have up, not just the ones it planned.
- **AOE heals now stand you up first**: with **Rest** on, a party dropping below the AOE threshold while you were resting meant the cast went out sitting down and the game just refused it. Sidekick now breaks rest first, the same as it always did for single-target healing.
- **`/anon` no longer stops Sidekick from doing anything**: with `/anon` on, the server hides your job and level from the party list, so Sidekick read you as level 0 and quietly skipped every ability. It now reads your own job and level directly from your character instead. Other players' hidden jobs show as *hidden* in `/sk panel`. Thanks to **Atsumu** and **Pax** for reporting the bug.
