...
### Changed
- **Buffs cast in a smarter order** — some buffs are worth casting first. **Composure** now goes up before Red Mage's other self-buffs, so those buffs pick up the longer duration Composure gives them; **Sublimation**, the **Geomancer** self-luopans, and **Refresh** are likewise nudged ahead of lower-priority buffs. Every other buff is unaffected. Thanks to **Plush** the feature idea and code.
...
- **Bard single-target songs raced ahead of area songs** — in **Pianissimo Fast Casting**, if an area (`[A]`) song was briefly on recast, Sidekick sang a single-target song anyway, which the area song then overwrote the moment it came back up. The single-target pass now holds until every configured area song is established first.  Thanks to **Sleazy** for reporting the bugg
- **Erase and Healing Waltz skipped Shock and Drown** — the two elemental damage-over-time debuffs (thunder and water) were missing from the removable list, so Sidekick left them up even though Erase (and Dancer's Healing Waltz) can strip them. Both are now recognized and cured alongside Burn, Frost, Choke, and Rasp.  Thanks again to **Sleazy** for reporting the bugg
