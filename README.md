...
### Added
- **New Pet Control section (PUP/SMN/BST)**: one section holding both pet features, each with its own checkbox. **Maneuver** lets you pick up to 3 elemental Maneuvers from three dropdowns and Sidekick keeps them applied, stacking duplicates if you pick the same element twice — works whether PUP is main or sub job. Alongside it, the "send my pet at my target" toggle — labeled with your job's own ability (**Deploy** for PUP, **Assault** for SMN, **Fight** for BST) — is opt-in and combat-only.
...
- **Beastmaster** (BST) — *pet-only support*
  ...
  - **Fight** (**Pet Control** section, opt-in, off by default): sends your pet at your current target whenever it doesn't already have one of its own. Combat-only — shared logic with Puppetmaster's Deploy and Summoner's Assault.
...
- **Puppetmaster** (PUP) — *pet-only support*
  ...
  - **Deploy** (**Pet Control** section, opt-in, off by default, its own checkbox — independent of Maneuver): sends the automaton at your current target whenever it doesn't already have one of its own. Combat-only — shared logic with Summoner's Assault and Beastmaster's Fight.
...
- **Summoner** (SMN)
  ...
  - **Assault** (**Pet Control** section, opt-in, off by default): sends your avatar at your current target whenever it doesn't already have one of its own. Combat-only — shared logic with Puppetmaster's Deploy and Beastmaster's Fight.
