...
- **Geomancer waits for a still target before dropping a bubble**: a luopan stays where it lands, so Sidekick now holds **Geo** debuffs while the battle target is on the move, and **Geo** enhancing spells while the party member you picked is walking — no more bubbles dropped behind someone who is about to run out of them. The cast goes out as soon as they stand still. **Indi** spells are untouched, since those follow their target. Thanks to **Benthere** for the feature idea.
