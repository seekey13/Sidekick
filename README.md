...
- **Custom song duration for level-75 Bards**: song+ duration gear makes your songs last longer than Sidekick can see, so it always waited for a song to *drop* before re-singing it. Set **Song Duration (s)** on `/sk panel` to just under your real duration and Sidekick re-sings each song on that timer instead — before it wears, so your party never loses the buff. **Troubadour** doubles the timer automatically while it's up. Set it to `0` (the default) for the old behavior. While a timer is running Sidekick trusts it over what it can see, so a song you lose some other way — a KO and raise, a Dispel — waits for that timer before it comes back; keep the number under your real duration. Thanks to **Sleazy** for the brainstorming session behind this feature.
- **Bard gets Nightingale and Troubadour**: both merit abilities were missing from Bard's buff list. Tick them on and Sidekick keeps them up like any other self buff (level 75, main job only, merits required).
