...
- **New floating widget (`/sk widget`)**: pops the top of the config window — your profile button, job/subjob line, the Start/Stop button, the status text and the **Track Target** button — out into a small window of its own that you can park anywhere. Your list of tracked targets stays in the config window. No title bar, no close button: `/sk widget` toggles it, drag the body to move it. While it's up those rows disappear from the config window, so you're never looking at two Start buttons. Remembered per character, so it comes back where you left it after a reload. Thanks to **Muziko** for the feature idea.
