...
- **`common.successf`** (green, `chat.success`) and a lucky-number callout: landing on a roll's `lucky` total prints `[Roll] Corsair's Roll (EXP+): LUCKY 5!` in green from the packet handler. It reports the hit only, not a stop: Lowest/Medium bank the lucky number, Highest rolls past it.
