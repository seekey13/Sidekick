...
### Removed
- **Attack Range Management**: Automated combat positioning via multisend follow commands
- **has_ammo() function**: Unused function for ammunition checking
...
- Removed obsolete attack range UI dropdown from configuration interface
- Updated header comments in core modules to remove "automation framework" references
...
- Movement/positioning automation
- Attack abilities
- Debuff application (offensive debuffs)
- Step stacking
- Roll automation
- Pet combat commands

### Technical Details
- Cleaned up Medic.lua to remove range management state and logic
- Simplified default_settings to remove automation-related options
- Updated config_ui.lua to remove attack range dropdown
- Removed has_ammo() from common.lua (unused after automation removal)
...
## Previous Version History (Pre-Support-Only Focus)

**Note:** Versions 1.0.0 through 2.9.0 represented a full automation framework with combat, tanking, nuking, and other features. These versions are preserved in CHANGELOG history for reference but are no longer the focus of this addon.
