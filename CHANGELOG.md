...
- **BLU set-spell gating**: Blue magic that isn't in the player's currently-equipped set-spell list is grayed with a *"Blue Magic not currently equipped"* tooltip and dropped by automation (`common.is_blue_magic_unequipped`, read from `get_equipped_blue_spells`, checked in `filter_abilities_by_level`). The row stays selectable — Medic never equips spells for you.
