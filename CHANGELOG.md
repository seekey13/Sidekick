...
### Fixed
- **Red Mage Composure now casts**: Composure's `id` (recast id) was `247`, another ability's timer, so the recast check read the wrong cooldown and Composure never auto-fired. Corrected to `50`. Thanks to **Dasaikuru [DS]** for reporting the bug.

- **Geomancer Indi/Geo MP costs corrected**: Many Indi, Geo, and Geo-bt spell `cost` values came from retail rather than CatsEyeXI's `spell_list` `mpCost`, so Sidekick could skip a spell it could actually afford or attempt one it couldn't. All geomancy costs now match the server (`spell_list.sql`). Thanks to **Tai** for reporting the bug.
