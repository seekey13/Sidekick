...
- **`common.successf`** (green, `chat.success`) and a lucky-number callout: landing on a roll's `lucky` total prints `[Roll] Corsair's Roll (EXP+): LUCKY 5! Bonus effect at full potency -- holding here.` in green from the packet handler, so the sudden stop in Double-Up has a visible reason. Accurate on every tier because every roll's `lucky` is ≤ 5, i.e. inside the zero-risk zone where `roll_strategy` stops on lucky regardless of tier.
