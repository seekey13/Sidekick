...
Generalizes packet-based buff tracking for Trusts and tracked targets into a timed-expiry system with base durations, extends it to debuffs and to alliance members / the pet, adds per-caster song slots, and stops removal spells (and cure-wake) from looping on targets whose buffs can't be read from memory.
