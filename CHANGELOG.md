...
- **Interrupted casts recorded a Trust buff that never landed**: an interrupt arrives as 0x028 category 4 (`spell_finish`) like a success, so `handle_buff_application()` popped the pending buff and suppressed that buff's recast for its full base duration. Category 4 with `Param == 28787` (`0x7073`) marks the interrupt and is now skipped.

- **Zone change clears a stuck casting lock**: zoning cancels the cast without ever sending the `spell_finish` that clears `is_casting`, and a stuck lock is self-sustaining (locked = no actions = no packet to clear it), so the `cast_timeout` backstop froze automation for its full 30 s. The 0x0A handler now calls `common.clear_casting_state()` alongside the existing Trust-buff / tracked-target clears.
