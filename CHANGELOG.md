...
- **Paladin self-buffs**: PLD gains its defensive job abilities and Reprisal. **Majesty** (70) carries `priority = 100` so it lands ahead of the other self-buffs — its Cure-potency bonus only counts on Cures cast while it's up — and is deliberately *not* `combat_only`, since it should already be active when the healing starts. **Fealty** (75, merit `ability_id = 157`), **Rampart** (62), **Sentinel** (30) and **Holy Circle** (5) are all `combat_only`: each is a short defensive cooldown, so firing them out of combat just burns the recast. **Reprisal** (61) is enhancing white magic (`spell_id = 97`, 24 MP), not a JA, so it goes through the spell recast table and picks up the enhancing-magic column buttons like any other white enhancing row.
