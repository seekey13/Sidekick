...
### Added
- **Black Mage (BLM) support**: Self-only automation. Self-buffs the elemental **Spikes** (Blaze Spikes 10, Ice Spikes 20, Shock Spikes 30 — grouped as `spikes`, single tier selectable via dropdown); self-heals with **Drain** (12) on the battle target; recovers MP with **Aspir** (25) on the battle target. Drain and Aspir are dark magic cast on `<bt>` and so are inherently combat-only; Drain is flagged `self_only` so the heal engine only ever fires it on the caster's own HP (never as a party heal), and prefers the highest-value Drain that fits the HP deficit. Thank you to Mythicangel, who reminded me BLM has buffs.
