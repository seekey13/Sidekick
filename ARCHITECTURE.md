...
- **Scholar stratagems & BST Ready**: `check_stratagem(job_def, settings, ability_key, ability)` returns the next stratagem JA to fire, `nil` (cast the spell), or `false` (skip). `spendable_stratagems(settings)` is the charge count a caller may actually spend: with **Hold a Stratagem for AOE** (`settings.stratagem_reserve_aoe`, off by default) on **and `heal_aoe_enabled` also on**, it subtracts one, so a lone charge reads as none and the reserve stays for Scholar's Accession AOE heal — which, being the reserve's own consumer, reads `game_state.stratagems` directly, as does the `/sk panel` display. `check_stratagem` and Scholar's `validate_ability` both read the adjusted count. The `heal_aoe_enabled` half of that condition is load-bearing rather than defensive: with AOE Healing off there is nothing to reserve *for*, and the checkbox that clears the reserve sits on an ability row `config.lua` only draws while the section is open **and** enabled — so a reserve left set would starve Addendum: White and every assigned precast with no visible control left to undo it. `effective_ability_cost(ability, settings, job_def)` applies an `mp_modifier` stratagem when the user **assigned** it *or* when its **buff is already up** — the server charges the modified cost to whichever spell consumes the buff, regardless of who planned it, and Accession's buff can arrive from `heal.execute_aoe` firing the JA itself or from the player pressing it by hand. It is one pass over `abilities.precast` rather than a nested loop over the assignment table, so a strat that is *both* assigned and active multiplies once instead of squaring (2.0x → 4x). The buff half also applies the magic colour / `magic_types` match the **S** popup uses when offering a stratagem (an assignment already carries it by construction), else a black Parsimony would halve a white Cure's budget; that match is the shared `stratagem_magic_matches(strat, ability)`, which `get_available_stratagems` in `components.lua` calls too, and `stratagem_applies`'s `spell_ids` gate is separate and applies to both halves — tested **last** in the chain, since it is a ~50-entry linear scan and this function runs per ability per tick *and* per UI frame. "Hold for Stratagem" (`settings.stratagem_hold[<key>]`) controls the can't-fire case: when held it returns `false` (skip until ready); otherwise `nil` (cast without the stratagem). `prune_unavailable_stratagems(job_def, settings)` drops assigned stratagems above the current SCH level on job/level change (bails on a transient level-0 read), fixing a high-level SCH assignment sticking after a downgrade to a lower level or `???/SCH`. Both stratagems and BST **Ready** are charge systems on one shared recast timer, so a single `charges_from_recast(recast_id, max, rate)` helper converts the remaining-until-full timer into available charges for both (`state.stratagems`, `state.ready_charges`). A stratagem entry with `recast_gate = true` (DRK **Nether Void** on the `absorb` group via the N button; BLU **Diffusion** per blue buff row via the D button; RUN **Embolden** per white enhancing row via the E button; SCH **Enlightenment** on the Addendum: White-gated rows via its own E button) names its button column with `column` and has no charge pool: it's gated on its own JA recast being ready and on `precast_permanently_usable` (on-level, meritted, not a main-job JA supplied by the subjob — so a de-level or a lost merit can't spam an unusable JA); when unavailable the spell casts without it (same hold-off rule). Job files list `recast_gate` strats **ahead of** the charge strats, so `missing[1]` being one means the tick spends no charge and any charge strat behind it is re-checked on a later tick.
...
- **`aoe_precast` pairing (SCH Accession)**: an `abilities.heal_aoe` entry flagged `aoe_precast` is a JA that heals nothing on its own — it makes the *next* single-target cure land on everyone in range. It is split out of the list handed to `first_command` (which would otherwise fire the bare JA as though it were the heal) and handled on its own path, keyed off the data flag rather than the job id. The party scan also tracks the lowest-HP eligible member, since the spell radiates from its **target**: the cure is aimed there and range-checked against it. The pass runs in three steps, in this order:
  ...
  1. **Buff already up** → cast the paired cure. **The threshold decision was made on the tick that spent the charge, and is not re-tested here**: step 3 stamps a module-local `precast_fired_at`, and a buff seen within `PRECAST_FOLLOWUP_WINDOW` (5 s, matching `automation.lua`'s `STRATAGEM_FOLLOWUP_TIMEOUT`) is taken as this module's own and casts unconditionally. Re-gating would strand the cure over a party that healed up during the JA animation, where the charge is already gone and the buff would be spent by `heal.execute` at double MP anyway. A buff with no stamp — an **S**-popup assignment firing Accession, or the player pressing it by hand — is *not* ours and keeps `below_count >= 1`, so it can't be hijacked into a full-HP party. A timestamp rather than a boolean, so it expires on its own and needs no clearing on the paths that never reach the cure. The cure is cast **directly**, deliberately bypassing `common.check_stratagem` — a stratagem the user assigned to Cure through the **S** popup would spend a second charge and break the lock.
  2. Otherwise, once `aoe_ready` holds, **`first_command(plain, …)`** — a real AOE heal outranks the precast: a free Curaga beats a charge plus double MP. No shipped job reaches this today (Accession is SCH 40 and CatsEyeXI caps a subjob at 37, so `/SCH` can never merge it into a job that already has Curaga), but the flag is generic and this is the order any future `aoe_precast` entry wants.
