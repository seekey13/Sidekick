...
- **Element auto-select**: `settings.auto_element_<group> = true` lets `lib/core/vanadiel.lua` steer `selected_<group>` to the tier whose `element` matches the environment. Ignored while `ungrouped_<group>` is set.
