...
- **Moving-target gate**: a luopan is placed where its target stands and then stays there, so both Geo casts are held while that target is walking — `Geo-bt` against `common.targets.get_bt()` in `geo.lua` (rate-limited `hold_log`, no state), `group = 'Geo'` against the selected ME/P1-P5 member in `buff.lua`'s single-target loop, after the zone/range resolution so an unresolved member never reaches the check. Both call `common.is_entity_moving`. `'Indi'` is exempt: the aura follows its target. No setting — a bubble dropped at a running target is never wanted.
