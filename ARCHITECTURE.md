...
**DRK Nether Void button** (`render_nether_void_button`): An **N** button in the Absorb group row's leading slot, drawn for DRK main at Nether Void's level; while the merit is not unlocked it renders disabled (grayed, click-locked, *"Not Learned"* tooltip), like an unlearned spell row. Clicking opens a popup with **Enable** (`stratagem_settings['absorb']['Nether Void']` — the automation fires Nether Void before the selected Absorb) and **Hold for Nether Void** (`stratagem_hold['absorb']`, the same hold key Scholar uses — ON skips the Absorb until Nether Void is ready; OFF, the default, casts the Absorb without it when the JA is on cooldown). Lit when enabled.
