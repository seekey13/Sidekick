...
- **Config window open state**: `ui_open` is stamped from `ui_config.is_visible()` in the `unload` handler and restored one-shot on the first tick settings exist (`state_restored` in `Sidekick.lua`). `widget_open` follows the same path for the floating widget (`/sk widget`), and is additionally stamped on toggle. The widget is a second window rendering `config.lua`'s `render_header(ctx)` (profile button + job line, Start/Stop + status + the Track Target button; the tracked-targets list stays in the config window); the config window skips that call while the widget is visible, so those rows render once per frame.
